  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-3xl p-5 border border-slate-200/90 shadow-sm space-y-3.5">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl bg-[#FAF6E6] border border-[#EFC548]/40 text-[#0C1326] flex items-center justify-center font-bold text-base shadow-xs">
              🎟️
            </div>
            <div>
              <h2 className="font-extrabold text-sm text-navy-900 tracking-tight">
                1960 청량로드 스탬프 여권
              </h2>
          ...
          <button
            ...
            className="px-2.5 py-1.5 rounded-xl bg-[#0C1326] text-[#EFC548] border border-[#EFC548]/40 text-[10px] font-bold hover:bg-[#131E3A] transition-all flex items-center gap-1 shadow-xs"
          >
            <QrCode size={12} className="text-[#EFC548]" />
            <span>스캔 인증</span>
          </button>
        </div>
          ...
          {TOUR_SPOTS.map((spot) => {
            ...
            return (
              <button
                ...
                className={`p-2.5 rounded-2xl border text-center flex flex-col items-center justify-center transition-all active:scale-95 ${
                  isCompleted
              ...
              >
                ...
                <span className="text-[10px] font-bold line-clamp-1 text-navy-900">
        ...
        <div className="text-[11px] text-slate-600 bg-slate-50 p-3 rounded-2xl border border-slate-100 flex items-center justify-between">
          <span>스탬프 현황: <strong>{completedCount}개 적립 완료</strong></span>
          <span className="text-[#7E5D0A] font-bold">다음 리워드까지 {Math.max(0, 3 - completedCount)}개</span>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between px-1">
          <h3 className="font-extrabold text-xs text-navy-900 flex items-center gap-1.5">
            <Gift size={14} className="text-[#7E5D0A]" />
            나의 모바일 할인 쿠폰 ({unusedCoupons.length}장 사용 가능)
          </h3>
          <span className="text-[10px] text-slate-400">현장 결제 시 제시</span>
        </div>
        ...
        <div className="space-y-2.5">
          {unusedCoupons.map((coupon) => (
            <div
              key={coupon.id}
              className="relative bg-white rounded-2xl border border-slate-200/90 overflow-hidden shadow-xs hover:shadow-md transition-all p-4"
            >
              {/* Top Row */}
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <h4 className="font-extrabold text-sm text-navy-900 mt-1.5">
                    {coupon.title}
                  </h4>
                </div>
                <span className="font-black text-base text-[#7E5D0A]">
                  {coupon.discount}
                </span>
              </div>
              ...
              <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Barcode size={28} className="text-slate-700" />
                  <span className="text-[10px] font-mono text-slate-400">{coupon.barcode}</span>
                </div>
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl bg-[#0C1326] hover:bg-[#131E3A] text-[#EFC548] border border-[#EFC548]/40 text-xs font-bold shadow-xs active:scale-95 transition-all"
                >
                  사용하기
                </button>
              </div>
          ...
          {usedCoupons.length > 0 && (
            <div className="pt-2 space-y-1.5 opacity-60">
              ...
              {usedCoupons.map((c) => (
                <div key={c.id} className="p-3 bg-slate-100 rounded-xl border border-slate-200 flex items-center justify-between text-xs">
                  <span className="line-through text-slate-400 font-medium">{c.title}</span>
                  <span className="text-[10px] bg-slate-200 px-2 py-0.5 rounded text-slate-600 font-bold">사용완료</span>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200/80 space-y-2 text-xs text-slate-700">
        <h4 className="font-extrabold text-xs text-navy-900 flex items-center gap-1.5">
          <AlertCircle size={14} className="text-[#EFC548]" />
          알아두면 유용한 청량리 장보기 팁
        </h4>
