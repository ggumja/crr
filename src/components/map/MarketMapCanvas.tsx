...
export const MarketMapCanvas: React.FC = () => {
  ...
  const renderMapContent = () => (
    ...
    >
      ...
      {showStorePins && filteredStores.map((store) => {
        ...
        return (
          ...
          >
            ...
            {/* Pin Badge Graphic */}
            <div className={`w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center shadow-md border-2 transition-all ${
              isSelected
            ...
            <div className={`absolute top-full left-1/2 -translate-x-1/2 mt-1 px-2 py-0.5 rounded-md bg-[#0C1326] text-white text-[9px] font-bold whitespace-nowrap pointer-events-none shadow-lg border border-[#EFC548]/40 z-30 transition-opacity ${
      ...
      {hoveredZone && (
        <div className="absolute bottom-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-[#0C1326]/90 backdrop-blur-sm text-white text-[11px] font-bold border border-[#EFC548]/40 shadow-md flex items-center gap-1.5 pointer-events-none z-30">
          ...
          <span className="text-[#EFC548] text-[10px]">· 클릭하여 점포 필터</span>
        </div>
      )}
    </div>
  );
    ...
    <div className="space-y-3">
      ...
      <div className="relative">
        <input
          ...
          placeholder="점포명, 품목(순대국, 쌍화차, 가마솥통닭, 과일) 검색..."
      ...
      <div className="flex items-center justify-between text-xs px-1">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="p-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:text-navy-900 hover:border-slate-300"
            title="지도 크게 보기"
          >
            <Maximize2 size={13} />
          </button>
      ...
      >
        ...
        <div className="absolute top-2.5 right-2.5 flex flex-col gap-1 z-30 bg-white/95 backdrop-blur-sm rounded-xl p-1 shadow-sm border border-slate-200/80">
          <button
            onClick={handleZoomIn}
            className="w-7 h-7 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-700 transition-colors"
            title="확대 (+)"
          >
            <ZoomIn size={14} />
          </button>
          ...
          {zoomLevel > 1 && (
            <button
              ...
              className="w-7 h-7 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-700 transition-colors text-[9px] font-bold"
              title="원래 배율 복원"
            >
              <RotateCcw size={12} />
            </button>
        ...
        {/* Map Legend Overlay */}
        <div className="absolute bottom-2.5 left-2.5 bg-white/95 backdrop-blur-sm rounded-xl px-2.5 py-1.5 text-[9px] border border-slate-200/80 text-slate-600 space-y-1 z-10 shadow-xs">
          <div className="flex items-center gap-1.5">
            <span className="w-2.5 h-2.5 rounded-full bg-[#EFC548] text-[#0C1326] font-black text-[8px] flex items-center justify-center">1</span>
            <span className="font-bold text-navy-900">미식 투어 스팟 (1~8번)</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-sm bg-[#EFC548]/30 border border-[#EFC548]" />
            <span>선택 구역 하이라이트</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2 pt-2">
        <div className="flex items-center justify-between px-1">
          <h3 className="font-extrabold text-xs text-navy-900">
            상권 주요 점포 ({filteredStores.length}곳)
          </h3>
          <span className="text-[10px] text-slate-400">지도 핀 클릭 시 위치 확인</span>
        </div>
            ...
            <div
              ...
              className={`p-3 rounded-2xl border transition-all cursor-pointer flex items-center justify-between shadow-xs ${
                activePreviewStore?.id === store.id
                  ? 'bg-[#FAF6E6] border-[#EFC548] ring-1 ring-[#EFC548]/40'
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="flex items-center gap-1.5">
                    ...
                    {store.tourSpotNumber && (
                      <span className="text-[9px] px-1.5 py-0.2 rounded bg-[#FAF6E6] text-[#7E5D0A] font-extrabold border border-[#EFC548]/40">
                        #{store.tourSpotNumber}
                      </span>
                    )}
                  </div>
                  <h4 className="font-bold text-xs text-navy-900 mt-0.5 line-clamp-1">{store.name}</h4>
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button
                  ...
                  className="px-2.5 py-1.5 rounded-lg bg-[#0C1326] text-white text-[10px] font-bold hover:bg-[#131E3A] transition-colors"
                >
                  상세
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex flex-col justify-between p-4 animate-in fade-in duration-200">
          <div className="flex items-center justify-between text-white pb-3 border-b border-white/10">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-extrabold text-sm">1960 청량로드 공식 가이드맵</h3>
            </div>
            <button
